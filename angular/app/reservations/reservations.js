    ...
    ['$scope', '$routeParams', '$location', '$mdDialog', '$window', '$moment', 'reservations', 'settings',
      function ($scope, $routeParams, $location, $mdDialog, $window, $moment, reservations, settings) {
        ...
        $scope.addPayment = function (ev) {
          $scope.reservation.payments.push({
            paymentType: 1,
            amount: 0
          })
          $scope.paymentAmountChanged()
        }

        $scope.paymentAmountChanged = () => {
          $scope.reservation.balance = $scope.reservation.payments.reduce((prev, cur) => {
            return prev + cur.amount
          }, 0)
        }
        $scope.removePayment = function (ev, payment) {
          var index = $scope.reservation.payments.indexOf(payment)
          $scope.reservation.payments.splice(index, 1)
          $scope.paymentAmountChanged()
        }
  ...
  .factory('reservations', ['$http', function ($http) {
    ...
    var out = {
      ...
      update: function (reservation) {
        return $http.post(urlBase + reservation._id, formatReservationOut(reservation))
      }
    }
    return out
  }])
